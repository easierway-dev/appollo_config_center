...
func GetNameSpaceInfo() (nameSpaceInfo []*capi.NamespaceInfo, err error) {
	// 获取全局配置
	...
	if nameSpaceInfo == nil {
		return nil,errors.New(apolloProperty.ClusterName + "nameSpacesInfo is nil")
	}
...
func GetSingleNameSpaceInfo(nameSpace string) (map[string]string,error){
	nameSpaceInfo,err := GetNameSpaceInfo()
	if err != nil {
		fmt.Println("err=", err)
		return nil,err
	}
	apolloKV := make(map[string]string)
	for i, info := range nameSpaceInfo {
		...
		if nameSpaceInfo[i].NamespaceName == "application" {
			for _, item := range info.Items {
				apolloKV[item.Key] = item.Value
			}
		}
	}
	if len(apolloKV) == 0{
		return nil,errors.New("apollo is nil")
	}
	return apolloKV,nil
}
